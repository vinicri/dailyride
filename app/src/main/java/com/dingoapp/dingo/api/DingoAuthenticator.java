...
public class DingoAuthenticator implements Authenticator {
    ...
    public Request authenticate(Route route, Response response) throws IOException {
        ...
        com.dingoapp.dingo.api.Response<User.OAuthToken> refreshTokenResponse = DingoApiOAuthService.getInstance().refreshToken(CurrentUser.getUser().getAuthToken().getRefreshToken());
        if(refreshTokenResponse.code() == com.dingoapp.dingo.api.Response.HTTP_200_OK){
            CurrentUser.getInstance().updateOAuthToken(refreshTokenResponse.body());
        }
        else if (refreshTokenResponse.code() == com.dingoapp.dingo.api.Response.HTTP_401_UNAUTHORIZED){
